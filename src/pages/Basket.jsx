...
import { clearPizzaActionCreator } from "../redux/Action/AddPizzaActionCreator";
import basketEmpty from '../assets/img/basketEmpty.svg'
import { Link } from "react-router-dom";


const Basket = () => {
    const addedPizzas = useSelector(({ addPizza }) => addPizza)
    const dispatch = useDispatch()
    const clearPizzas = () => {
        if (window.confirm("Вы действительно хотите очистить корзину?")) {
            dispatch(clearPizzaActionCreator())
        }
    }



    // console.log(addedPizzas.items[7].items[0].name)
    const keys = Object.keys(addedPizzas.items);

    const items = keys.map(key => {
        return {
            items: addedPizzas.items[key].items[0],
            TotalCount: addedPizzas.items[key].TotalCount,
            TotalPrice: addedPizzas.items[key].TotalPrice
        }
    })
    return (
        <div class="content">
            <div class="container container--cart">
                {items.length ? <div class="cart">
                    <div class="cart__top">
                        ...
                        <div class="cart__clear">
                            ...
                            <span onClick={clearPizzas}>Очистить корзину</span>
                        </div>
                    </div>
                    ...
                    <div class="cart__bottom">
                        <div class="cart__bottom-details">
                            <span> Всего пицц: <b>{addedPizzas.TotalCount} шт.</b> </span>
                            <span> Сумма заказа: <b>{addedPizzas.TotalPrice} ₽</b> </span>
                        </div>
                        <div class="cart__bottom-buttons">
                            ...
                            <div class="button pay-btn" onClick = {() => alert('Вы купили пиццу, вау!')}>
